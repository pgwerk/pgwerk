...
async def _create_wrk() -> Wrk:
    dsn = os.environ["WRK_DSN"]
    app = Wrk(dsn)
    await app.connect()
    return app


async def _close_wrk(wrk: Wrk) -> None:
...
def create_app(
    ...
    Pass an already-connected ``Wrk`` instance, or set ``WRK_DSN`` in the
    ...
    if wrk is not None:
        ...
        async def _shutdown() -> None:
            ...
            if "wrk" in _state:
                await _close_wrk(_state["wrk"])

        async def _get_wrk() -> Wrk:
            return _state["wrk"]

        dependencies["wrk"] = Provide(_get_wrk, use_cache=True, sync_to_thread=False)
