...
class WrkConfig:
    ...
    Attributes:
        ...
        prefix: Prefix applied to every wrk table name (default ``"_wrk"``),
            producing tables like ``_wrk_jobs``, ``_wrk_worker``, etc.
        ...
        heartbeat_interval: How often (seconds) a running worker updates
            its heartbeat timestamp in ``_wrk_worker``.
        poll_interval: How often (seconds) the polling loop checks
            ``_wrk_jobs`` for new work when no LISTEN/NOTIFY wake-up
            arrives.
        ...
        ephemeral_tables: Use ``UNLOGGED`` tables for ``_wrk_worker`` and
            ``_wrk_worker_jobs``. Faster writes; data is lost on a crash,
