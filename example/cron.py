"""
Runs a CronScheduler with a handful of example schedules.
...
from pgwerk import CronScheduler
...
scheduler.register(heartbeat_ping, interval=30, queue="default")

# Runs every 60 seconds — simulates a metrics flush.
scheduler.register(flush_metrics, interval=60, queue="default")

# Runs every 5 minutes — simulates a stale-session cleanup.
scheduler.register(cleanup_expired_sessions, interval=300, queue="default", kwargs={"older_than_days": 30})

# Runs on a cron expression — every day at 08:00 UTC.
scheduler.register(daily_report, cron="0 8 * * *", queue="default", timeout=300)
